...
public class S120876 {
    ...
    public static int solution(int[][] lines) {
        ...
        for (int[] line : lines) {
            // 1차원 배열 line의 범위를 -100 ~ 100 -> 0 ~ 200 으로 치환해준다.
            for (int j = (line[0] + 100); j < (line[1] + 100); j++) {
                // 새로운 배열에 인덱스로 저장해준다.
                arr[j]++;
            }
        }
        // 특정 요소의 값이 2 이상이면, 겹친 좌표이므로 겹친 좌표의 수를 모두 더해주면 겹친 선분의 길이가 나온다.
        // k = 0 부터 arr.length -1 까지 실행문을 반복한다.
        for (int k = 0; k < arr.length; k++) {
            // arr 배열의 k번째 인덱스 값이 1보다 크면 (2 이상이면)
            if (arr[k] > 1) {
                // 겹친 부분 수를 담을 변수 answer의 수를 증가
                answer++;
            }
        }
